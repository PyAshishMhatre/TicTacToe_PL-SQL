------------------------------CREATING GRID------------------------------------------------
DROP TABLE TICTAC; -- TIC TAC TOE GRID
DROP TABLE TRACK; -- TABLE TO HOLD LAST PLAYED SYMBOL 'X' OR '0'

CREATE TABLE TICTAC(
ROW_NUM NUMBER GENERATED ALWAYS AS IDENTITY START WITH 1 INCREMENT BY 1 MAXVALUE 3, -- 3 X 3 GRID CONST.
...
INSERT INTO TICTAC(COL1,COL2,COL3) -- PREFILL WITH NULL VALUES
...
INSERT INTO TRACK(SYMBOL) -- PREFILL WIRH NULL VALUES
VALUES(NULL);

SELECT * FROM TICTAC; -- PRINT SAMPLE GRID
SELECT * FROM TRACK; -- PRINT SAMPLE TRACKER TABLE
...
BEGIN
    ...
    IF (:OLD.COL1 <> :NEW.COL1) OR (:OLD.COL2 <> :NEW.COL2) OR (:OLD.COL3 <> :NEW.COL3) THEN -- CHECK THAT A 'X' SHOULD NOT OVERWRITE '0' OR VICE VERSA
    
            DBMS_OUTPUT.PUT_LINE('TRYING TO ENTER IN SAME CELL WITH DIFFERENT VALUE');  --PRINT ERROR MESSAGE
            
            raise_application_error(-20111,'TRYING TO ENTER IN SAME CELL WITH DIFFERENT VALUE'); -- RAISE USER APPLICATION ERROR '-20111' 
    END IF;
...
BEGIN
        ...
        IF UPDATING('SYMBOL') THEN 
            ...
            IF (:OLD.SYMBOL <> :NEW.SYMBOL) OR (:OLD.SYMBOL IS NULL) THEN --ALLOW UPDATE WHEN LAST PLAYED AND CURRENT PLAYED SYMBOL DOES NOT MATCH
            
            DBMS_OUTPUT.PUT_LINE('LAST PLAYED --> ' || :NEW.SYMBOL);
            
            ELSE 
            
            raise_application_error(-20113,'SAME PLAYER PLAYING AGAIN'); -- RAISE ERROR IF SAME PLAYER 'X' OR '0' PLAYS BACK TO BACK
            DBMS_OUTPUT.PUT_LINE('PLAYER ' || :NEW.SYMBOL || ' IS TRYING TO PLAY AGAIN'); -- RAISE APPLICATION ERROR '-20113'
...
CREATE OR REPLACE PROCEDURE PLAY(SYM IN VARCHAR, ROW_NO IN NUMBER, COL_NO IN NUMBER) AS -- TAKE SYMBOL, ROW NUMBER, COLUMN NUMBER AS INPUT
  -- VARIABLES
        A1 VARCHAR(1);
        A2 VARCHAR(1);
        A3 VARCHAR(1);
