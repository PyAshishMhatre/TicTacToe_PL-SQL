...
BEGIN
    ...
    IF (:OLD.COL1 IS NULL AND :NEW.COL1 IS NOT NULL) OR (:OLD.COL2 IS NULL AND :NEW.COL2 IS NOT NULL) OR (:OLD.COL3 IS NULL AND :NEW.COL3 IS NOT NULL) THEN
            
            DBMS_OUTPUT.PUT_LINE('VALID MOVE PROCEED WITH GAME');
    
    ELSIF (:OLD.COL1 <> :NEW.COL1) OR (:OLD.COL2 <> :NEW.COL2) OR (:OLD.COL3 <> :NEW.COL3) THEN 
            --PRINT ERROR MESSAGE
    ...
    ELSE 
            -- RAISE USER APPLICATION ERROR '-20112 FOR ANY INVALID MOVE'
            DBMS_OUTPUT.PUT_LINE('INVALID MOVE');
            raise_application_error(-20112,'TRYING TO ENTER IN SAME CELL WITH SAME VALUE');
    END IF;
